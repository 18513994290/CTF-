...
/**
  ...
  */
   public static async function genAllBatchs() {
    ...
    $db = await self::genDb();
  // $result = await $db->queryf("SELECT id,start_ts,end_ts,batch_number,case status when '0' then '未开始' when '1' then '进行中' when '2' then '已结束' end as status FROM batchs ORDER BY id");	
            $result = await $db->queryf('SELECT * FROM batchs ORDER BY id');	
