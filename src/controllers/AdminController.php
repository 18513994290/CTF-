...
class AdminController extends Controller {
  ...
  private async function genGenerateBatchsSelect(
    ...
    $select =
      <select class="not_configuration" name="batch_number" disabled={true} />;
    if ($selected =='0') {
      $select->appendChild(
        <option value="0" >{tr('Auto')}</option>,
      );
      }else{
      $select->appendChild(
     ...
     $countries = await Attachment::genAllAvailableBatchs();
      foreach ($countries as $country) {
            $select->appendChild(
            <option value="0">
               {$country['batch_number']}
           </option>,
           );
         }
      return $select;
  ...
  private async function genGenerateCountriesSelect(
    ...
    if ($selected === 0) {
      ...
      $country = await Attachment::genBatch(intval($selected));
      $select->appendChild(
        <option value={strval($country[0]['id'])} selected={true}>
          {$country[0]['batch_number']}
        </option>,
      );
    }
   $countries = await Attachment::genAllAvailableBatchs();
    ...
    foreach ($countries as $country) {
      $select->appendChild(
        <option value={strval($country['id'])}>
          {$country['batch_number']}
        </option>,
      );
    }
  ...
  public async function genRenderQuizContent(): Awaitable<:xhp> {
   ...
   //print_r($countries_select);    
    ...
    foreach ($quizes as $quiz) {
      ...
      $countries_select =
        //await $this->genGenerateBatchsSelect($quiz->getBatchNumber()); // TODO: Combine Awaits
  ...
  public async function genRenderFlagsContent(): Awaitable<:xhp> {
    ...
    foreach ($flags as $flag) {
      ...
      list($countries_select, $level_categories_select) = await \HH\Asio\va(
       // $this->genGenerateBatchsSelect($flag->getBatchNumber()),
  ...
  public async function genRenderBasesContent(): Awaitable<:xhp> {
    ...
    foreach ($all_base_levels as $base) {
      ...
      list($countries_select, $level_categories_select) = await \HH\Asio\va(
        ...
        //$this->genGenerateBatchsSelect($base->getBatchNumber()),
        ...
        <nav class="admin-nav-links row-fluid">
          <ul>
            ...
            <li>
             ...
             <li>
              <a href="/index.php?p=admin&page=batch">
            ...
            <li>
              <a href="/index.php?p=admin&page=audit">{tr('Base Audit')}</a>
            </li>
            <li>
